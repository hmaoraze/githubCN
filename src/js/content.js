const allData = [
  ...
  [`Enter your user account's verified email address and we will send you a password reset link.`, `请输入您用户账户已验证的电子邮件地址，我们将向您发送密码重置链接。`],
  [`Email`, `电子邮件`],
  ...
  [`Password should be at least 15 characters OR at least 8 characters including a number and a lowercase letter.`, `密码长度至少为 15 个字符，或者至少为 8 个字符且包含一个数字和一个小写字母。`],
  [`Username`, `用户名`],
  [`Username may only contain alphanumeric characters or single hyphens, and cannot begin or end with a hyphen.`, `用户名只能包含字母数字字符或单个连字符，且不能以连字符开头或结尾。`],
  [`Your Country/Region`, `您的国家/地区`],
  [`For compliance reasons, we're required to collect country information to send you occasional updates and announcements.`, `出于合规原因，我们需要收集国家/地区信息，以便向您发送不定期的更新和通知。`],
